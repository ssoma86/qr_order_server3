...
@RequestMapping("/stuff/store")
public class StuffControllerForStore {
	...
	public void add(@Validated({ StuffValid.class, StuffInfoValid.class }) @RequestBody Stuff stuff,
		...
		stuff.setStore(user.getStore());
		stuff.setDefaultLan(user.getStore().getDefaultLan());
	...
	@GetMapping
	@JsonView(StuffControllerJsonView.class)
	public List<Stuff> search(String stuffNm, String useYn,
			@SessionAttribute(value = "sa_account", required = false) Account account) {
		
		User user = userService.findOne(account.getAccountId());
		
		return service.findStuffList(user.getStore().getStoreCd(), stuffNm, useYn);
	}
